// console/_components/agents/TaskPanel.tsx
'use client';
...
// Backend is ONLY used for triggering runs — all CRUD goes to Supabase directly
...
function StatusDot({ status }: { status: Run['status'] }) {
  const colors: Record<string, string> = {
    queued: '#aaa', running: '#f59e0b', completed: '#1a7f5a', failed: '#c0392b',
  };
  return (
...
function TaskCard({ task, orgId, onViewRun, onDelete }: {
    ...
    <div style={{ background: '#fff', border: '1px solid #e3e1dc', borderRadius: 6, overflow: 'hidden' }}>
      <div style={{ padding: '1rem 1.25rem', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontSize: '0.875rem', fontWeight: 500, color: '#111', margin: '0 0 0.25rem', letterSpacing: '-0.01em' }}>
            {task.name}
          </p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
          {latestRun && (
            <button onClick={() => onViewRun(latestRun.id)}
              style={{ background: 'none', border: '1px solid #ddd', color: '#555', padding: '0.3rem 0.65rem', fontSize: '0.72rem', fontFamily: "'DM Mono', monospace", cursor: 'pointer', borderRadius: 4, display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
          ...
          {!confirm ? (
            <button onClick={() => setConfirm(true)}
              style={{ background: 'none', border: '1px solid #ddd', color: '#888', padding: '0.3rem 0.6rem', fontSize: '0.72rem', fontFamily: "'DM Sans', sans-serif", cursor: 'pointer', borderRadius: 4, transition: 'border-color 0.12s, color 0.12s' }}
      ...
      {expanded && runs.length > 0 && (
        <div style={{ borderTop: '1px solid #f0eeea', padding: '0.75rem 1.25rem' }}>
          <p style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.62rem', color: '#aaa', letterSpacing: '0.06em', textTransform: 'uppercase', margin: '0 0 0.6rem' }}>
            Run history
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
            {runs.slice(0, 8).map(run => (
              <div key={run.id} onClick={() => onViewRun(run.id)}
                ...
                <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.65rem', color: '#555', flex: 1 }}>
                  {new Date(run.created_at).toLocaleString()}
                </span>
                <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.62rem', color: '#aaa', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                  {run.status}
                </span>
              </div>
            ))}
...
export default function TaskPanel({ agent, orgId, userId, onBack }: Props) {
  ...
  if (viewRunId) {
    return <RunViewer runId={viewRunId} onClose={() => setViewRunId(null)} />;
  }
  ...
  return (
    <div style={{ padding: '2.5rem 2rem', width: '100%' }}>
      ...
      {agent.description && (
        <p style={{ fontSize: '0.82rem', color: '#666', margin: '0 0 1.75rem 2.25rem', lineHeight: 1.5 }}>
          {agent.description}
        </p>
      )}

      <div style={{ marginTop: agent.description ? 0 : '1.75rem' }}>
        {showForm && (
        ...
        {loading ? (
          ...
          <div style={{ border: '1.5px dashed #ddd', borderRadius: 6, padding: '2rem' }}>
            <p style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.72rem', color: '#aaa', letterSpacing: '0.06em', textTransform: 'uppercase', margin: '0 0 0.4rem' }}>No tasks yet</p>
            <p style={{ fontSize: '0.825rem', color: '#777', margin: '0 0 1.1rem', lineHeight: 1.6 }}>
              Create a task with an instruction for this agent to run.
            </p>
