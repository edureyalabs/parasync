...
export default function GeneralSettings({ userEmail, userId, onLogout }: GeneralSettingsProps) {
  ...
  const handleAvatarChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('/biz/api/profiles/upload-avatar', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleBusinessActivation = async () => {
    ...
    try {
      ...
      setProfile({ ...profile, is_biz: true });
      setMessage({ 
        type: 'success', 
        text: 'Business account activated successfully' 
      });
      setTimeout(() => setMessage(null), 3000);
    } catch (error: any) {
      setMessage({ type: 'error', text: error.message || 'Failed to activate business account' });
    }
  };
    ...
    <div className="bg-white rounded-lg shadow-md">
      ...
      <div className="p-6 space-y-6">
        ...
        <BioSection 
          ...
          onUpdate={(newBio) => setProfile({ ...profile, bio: newBio })}
          onMessage={setMessage}
        />

        {/* Business Account Section */}
        <div className="pt-6 border-t border-gray-200">
          <h3 className="text-lg font-semibold text-gray-800 mb-3">Business Account</h3>
          {profile.is_biz ? (
            <div className="p-4 bg-green-50 rounded-lg border border-green-200">
              <div className="flex items-center gap-2">
                <Check className="text-green-600" size={20} />
                <div>
                  <p className="font-medium text-green-800">Business account is active</p>
                  <p className="text-sm text-green-700 mt-1">
                    You have access to all business features and tools
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <div>
                <p className="font-medium text-gray-700">Activate business account</p>
                <p className="text-sm text-gray-600 mt-1">
                  Get access to business features and tools
                </p>
              </div>
              <button
...
// Bio Component
function BioSection({ userId, initialBio, onUpdate, onMessage }: {
  userId: string;
  initialBio: string;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      onUpdate(bio);
      onMessage({ type: 'success', text: 'About section updated successfully' });
      setTimeout(() => onMessage(null), 3000);
    } catch (error: any) {
      onMessage({ type: 'error', text: error.message || 'Failed to update about section' });
    } finally {
      setSaving(false);
    }
  };
